Public Class Final
    ...
    Private Sub HandleInvalidInput(e As KeyPressEventArgs)
        ...
        ElseIf e.KeyChar < "0" Or e.KeyChar > "9" Then
            e.Handled = True
            Beep()
            MessageBox.Show("Wager must be a positive integer.")
        End If
    End Sub
    ...
    Private Sub okBtn_Click(sender As Object, e As EventArgs) Handles okBtn.Click
        If _mainForm.Team1 < team1Txt.Text Then
            team1Err.SetError(team1Txt, "Wager cannot be larger than the current score!")
            _error = True
        End If

        If _mainForm.Team2 < team2Txt.Text Then
            team2Err.SetError(team2Txt, "Wager cannot be larger than the current score!")
            _error = True
        End If

        If _mainForm.Team3 < team3Txt.Text Then
            team3Err.SetError(team3Txt, "Wager cannot be larger than the current score!")
            _error = True
        End If

        If _mainForm.Team4 < team4Txt.Text Then
            team4Err.SetError(team4Txt, "Wager cannot be larger than the current score!")
            _error = True
        End If

        If _mainForm.Team5 < team5Txt.Text Then
            team5Err.SetError(team5Txt, "Wager cannot be larger than the current score!")
            _error = True
        End If

        If Not _error Then
            _mainForm.Team1Wager = team1Txt.Text
            _mainForm.Team2Wager = team2Txt.Text
            _mainForm.Team3Wager = team3Txt.Text
            _mainForm.Team4Wager = team4Txt.Text
            _mainForm.Team5Wager = team5Txt.Text
            Me.Close()
            _mainForm.Show()
